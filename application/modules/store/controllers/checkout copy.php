...
class Checkout extends Controller {
	...
	function buyerinfo(){
		...
		if($this->cart->total_items() != 0){
			...
			if($login_data && !$this->session->userdata('customer_info')){
			...
			$data = array(
				...
				'countries' => $q->result(),
				'buyer_data' => $this->session->userdata('customer_info'),
				'ship_data'  => $this->session->userdata('ship_to_info'),
				//'loadSide' => false
				);
	...
	if(!$this->input->post('register') || $this->input->post('register') == null ){
			...
			if($this->input->post('different_address') != 1 || !$this->input->post('different_address') || $this->input->post('different_address') == null ){
				...
				$ship_to_data = array('ship_to_info' => $ship_to_info);
