	...
	<div class="form-Ui box2">
		<form action="" method="post" accept-charset="utf-8">
			<div class="inputSet">	
					...
					<div class="input"><input name="name" type="text" value="<?=$conf->name;?>" /></div>
					<div class="clear"></div>
			</div>
				<div class="inputSet">	
						<div class="label"><span>Description</span></div>
						<div class="input"><textarea name="description"><?=$conf->description?></textarea></div>
						<div class="clear"></div>
				</div>
			...
			<div class="item_conf mt10">
				...
				<div class="itemList mt20">
					<?foreach(json_decode($conf->config_object) as $item=>$value):
					
					?>
					<div class="item">
							<div class="inputSet left grid_400"><div class="label"><span><?=str_replace('_',' ', $item);?></span></div><div class="input"><input class="object_name" name="obj_<?=$item;?>" type="text" value="<?=$value;?>" /></div><div class="clear"></div></div>
							<div class="right">
								<span class="button del_obj_item">Delete</span>
							</div>
						<div class="clear"></div>
					</div>
					<?endforeach;?>
			...
			<br class="clear"/>
			<div class="ctr">
			<input type="submit" class="button" name="update"value="Continue &rarr;">
			</div>
	...
	$(document).ready(function(){
		...
		$('.addto').live('click',function(){
			...
			if(new_name == 'item name' && $.inArray(field_name, store_object) > -1 ){
				...
				var  new_item_el = '<div class="item"><div class="inputSet left grid_400"><div class="label"><span>'+new_name+'</span></div><div class="input"><input class="object_name" name="'+field_name+'" type="text" value="'+new_value+'" /></div><div class="clear"></div></div><div class="right"><span class="button">Delete</span></div><div class="clear"></div></div>';
