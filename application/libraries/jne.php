...

{
	function Jne(){
		...
		$this->url_getcity = 'http://www.jne.co.id/tariff.php?';
		$this->url_getrate = 'http://www.jne.co.id/index.php?mib=tariff&amp;lang=IN';
		$this->base_from_code = 'Q0dLMTAwMDBK';// jakarta base
		...
		if (!$result) {
			...
			curl_close($ch);
				...
				if(strpos($result, "<tr class='trfC'>") == true){
						...
						$rate = $this->_ci->addon_store->rate();
						$site_currency = $this->_ci->addon_store->currency();
						$index = 0;
						...
						foreach($result as $r){
							...
							$data[$index]['type'] = $r[0];
							$data[$index]['formated_rate'] = $this->_ci->addon_store->show_price($cost);
	...
	function test(){
		$data = array(
			'rate' => $this->_ci->addon_store->rate(),
			'currency' => $this->_ci->addon_store->currency()
		);
		return $data;
	}
